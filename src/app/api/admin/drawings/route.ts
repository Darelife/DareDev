...
export async function GET(request: NextRequest) {
  try {
    const rate = checkRateLimit(
      `admin:drawings:get:${getClientIp(request.headers)}`,
      30,
      60_000,
    );
    if (!rate.allowed) {
    ...
    const { data, error } = await supabase
      .from("drawings")
      .select(
        "id, title, created_by, created_at, updated_at, drawing_access(user_id)",
      )
...
export async function POST(request: NextRequest) {
  try {
    const rate = checkRateLimit(
      `admin:drawings:post:${getClientIp(request.headers)}`,
      20,
      60_000,
    );
    if (!rate.allowed) {
    ...
    if (userIds.length > 0) {
      const accessRows = userIds.map((userId) => ({
...
export async function PUT(request: NextRequest) {
  try {
    const rate = checkRateLimit(
      `admin:drawings:put:${getClientIp(request.headers)}`,
      20,
      60_000,
    );
    if (!rate.allowed) {
    ...
    const body = await request.json();
    const drawingId =
      typeof body?.drawingId === "string" ? body.drawingId.trim() : "";
    const userIds = normalizeUserIds(body?.userIds);
    ...
    return NextResponse.json(
      { success: true, drawingId, userIds },
      { status: 200 },
    );
  } catch (error) {
